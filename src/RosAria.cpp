...
class RosAriaNode
{
  ...
  public:
    ...
    void cmdvel_raw_cb( const geometry_msgs::TwistConstPtr &);
  ...
  protected:
    ...
    ros::Publisher omegar_pub;
    ros::Publisher omegal_pub;
    ...
    std_msgs::Float64 omegar;
    std_msgs::Float64 omegal;
  ...
  publish_aria_lasers(false)
{
  ...
  velr_pub = n.advertise<std_msgs::Float64>("vel_r",1000);
  vell_pub = n.advertise<std_msgs::Float64>("vel_l",1000);
  omegar_pub = n.advertise<std_msgs::Float64>("omega_r",1000);
  omegal_pub = n.advertise<std_msgs::Float64>("omega_l",1000);
...
int RosAriaNode::Setup()
{
  ...
  cmdvel_raw_sub = n.subscribe( "cmd_vel_raw", 1, (boost::function <void(const geometry_msgs::TwistConstPtr&)>)
      boost::bind(&RosAriaNode::cmdvel_raw_cb, this, _1 ));
...
void
RosAriaNode::cmdvel_raw_cb( const geometry_msgs::TwistConstPtr &msg)
{
  veltime = ros::Time::now();
  ROS_INFO( "new speed: [%0.2f,%0.2f](%0.3f)", msg->linear.x*1e3, msg->angular.z, veltime.toSec() );
  ...
  double vr, vl;
  vr = 1.0*v + ((a*a+b*b)/b)*omega;
  vl = 1.0*v - ((a*a+b*b)/b)*omega;
  // printf("vr: %f\nvl: %f\n\n", vr, vl);

  // long int enc_r =  robot->getRightEncoder();
  // long int enc_l =  robot->getLeftEncoder();
  // printf("enc_r: %ld\nenc_l: %ld\n\n", enc_r, enc_l);

  // double corr = 0.9890342892139209;

  // double vr_measured = robot->getRightVel()/1e3/0.9890342892139209;
  // double vl_measured = robot->getLeftVel()/1e3/0.9890342892139209;
  // // printf("vr_measured: %f\nvl_measured: %f\n\n", vr_measured, vl_measured);

  //Pioneer do Luciano
  double calib = 0.8495; //Measured by Adriano and Victor
  double vr_measured = calib*robot->getRightVel()/1e3;
  double vl_measured = calib*robot->getLeftVel()/1e3;
  ...
  velr.data = vr_measured;
  velr_pub.publish(velr);
  vell.data = vl_measured;
  vell_pub.publish(vell);

  double radius = 0.098;
  omegar.data = vr_measured/radius;
  omegar_pub.publish(omegar);
  omegal.data = vl_measured/radius;
  omegal_pub.publish(omegal);
  
  
  robot->setVel2(vl*1e3/calib,vr*1e3/calib);
